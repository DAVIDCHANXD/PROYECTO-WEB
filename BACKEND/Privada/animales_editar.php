<?php
// BACKEND/Privada/animales_editar.php
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ($url_imagen !== '') {
                $sqlFotoSel = "SELECT id_foto FROM fotos_animal 
                               ...
                               LIMIT 1";
                $stmtSel = $pdo->prepare($sqlFotoSel);
                $stmtSel->execute([':id_animal' => $id]);
                ...
                if ($foto) {
                    ...
                    $stmtUpd = $pdo->prepare($sqlFotoUpd);
                    $stmtUpd->execute([
                        ':url'     => $url_imagen,
            ...
            header('Location: animales_listar.php?msg=editar_ok');
            exit;
    ...
    try {
        ...
        $sqlFotoSel = "SELECT url FROM fotos_animal 
                       WHERE id_animal = :id_animal AND es_principal = 1
                       LIMIT 1";
        $stmtFoto = $pdo->prepare($sqlFotoSel);
        $stmtFoto->execute([':id_animal' => $id]);
        $foto = $stmtFoto->fetch(PDO::FETCH_ASSOC);
        $url_imagen = $foto['url'] ?? '';

    } catch (PDOException $e) {
        die('Error al obtener el animal: ' . $e->getMessage());
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Editar animal - AdoptaConAmor</title>
...
<div class="container py-4">
        ...
        <form method="post">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="id_tipo" name="id_tipo" required>
                    ...
                    <option value="1" <?= selected($animal['id_tipo'], 1) ?>>Perro</option>
                    <option value="2" <?= selected($animal['id_tipo'], 2) ?>>Gato</option>
                    <option value="3" <?= selected($animal['id_tipo'], 3) ?>>Conejo / Pequeños mamíferos</option>
                    <option value="4" <?= selected($animal['id_tipo'], 4) ?>>Ave</option>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="id_tamano" name="id_tamano" required>
                    <option value="">-- Selecciona --</option>
                    <option value="1" <?= selected($animal['id_tamano'], 1) ?>>Muy pequeño</option>
                    <option value="2" <?= selected($animal['id_tamano'], 2) ?>>Pequeño</option>
                    <option value="3" <?= selected($animal['id_tamano'], 3) ?>>Mediano</option>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="id_estado_salud" name="id_estado_salud" required>
                    <option value="">-- Selecciona --</option>
                    <option value="1" <?= selected($animal['id_estado_salud'], 1) ?>>Saludable</option>
                    <option value="2" <?= selected($animal['id_estado_salud'], 2) ?>>Vacunas en proceso</option>
                    <option value="3" <?= selected($animal['id_estado_salud'], 3) ?>>En recuperación</option>
                    <option value="4" <?= selected($animal['id_estado_salud'], 4) ?>>Tratamiento especial</option>
                    <option value="5" <?= selected($animal['id_estado_salud'], 5) ?>>Condición crónica</option>
                    <option value="6" <?= selected($animal['id_estado_salud'], 6) ?>>Observación</option>
    ...
    <?php else: ?>
        <div class="alert alert-danger">No se encontró el animal.</div>
